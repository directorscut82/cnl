...
namespace cnl {
    namespace _impl {
        // wide_integer_rep
        template<int Digits, typename Narrowest, bool SupportMultiply, typename Enable = void>
        struct wide_integer_rep;

        template<int Digits, typename Narrowest, bool SupportMultiply>
        struct wide_integer_rep<Digits, Narrowest, SupportMultiply, enable_if_t<(max_digits<Narrowest>::value>=Digits)>>
                : set_digits<Narrowest, max(Digits, digits<Narrowest>::value)> {
        };

        template<int Digits, typename Narrowest, bool SupportMultiply>
        struct wide_integer_rep<Digits, Narrowest, SupportMultiply, enable_if_t<(max_digits<Narrowest>::value<Digits)>>
                : optimal_multiword_integer<Digits, Narrowest, SupportMultiply> {
        };

        template<int Digits, typename Narrowest, bool SupportMultiply = true>
        using wide_integer_rep_t = typename wide_integer_rep<Digits, Narrowest, SupportMultiply>::type;
